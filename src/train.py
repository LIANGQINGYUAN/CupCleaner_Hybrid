...
def load_and_cache_gen_data(args, data_dir, pool, tokenizer, split_tag, only_src=False, is_sample=False):
    ...
    if split_tag == 'train':
        ...
        examples = read_examples(data_dir, split_tag, tokenizer, args)
    else:
...
def eval_bleu_epoch(args, eval_data, eval_examples, model, tokenizer, split_tag, criteria, accelerator):
    ...
    dev_accs, predictions, targets, sources = [], [],[], []
    with open(output_fn, 'w') as f, open(gold_fn, 'w') as f1, open(src_fn, 'w') as f2:
        for pred_nl, gold in zip(pred_nls, eval_examples):
            ...
            predictions.append(pred_nl.strip())
            targets.append(gold.target.strip())
            sources.append(gold.source.strip())
    ...
    result = {'em': round(tokenized_xmatch(targets,predictions)[0],2), 'bleu': round(compute_bleu_scores(targets,predictions, 'comment_update')[0],2)}
    accelerator.print("***** Eval results *****")
    for key in sorted(result.keys()):
        accelerator.print(f"  {key} = {str(round(result[key], 4))}")
    return result

def truncate_ids(ex_id_list):
    if 2 in ex_id_list:
        return ex_id_list[:ex_id_list.index(2)+1]
    else:
        return ex_id_list
...
def train(args, model, tokenizer):
    ...
    if args.do_train:
        ...
        for cur_epoch in range(int(args.epochs)):
            ...
            for step, batch in enumerate(bar):
                ...
                with torch.no_grad():
                    ...
                    # exs_loss = loss_fct(lm_logits.view(-1, lm_logits.size(-1)), target_ids.view(-1))
                    # batch_loss = exs_loss.view(target_ids.size(0), -1)
                    # batch_loss_mean = batch_loss.mean(-1).tolist()
                    # # preds
                    predids = torch.argmax(torch.softmax(lm_logits, dim=-1), dim=-1).tolist()

                    active_loss = target_ids.ne(0)
                    shift_logits = lm_logits.contiguous()
                    shift_labels = target_ids.contiguous()
                    exs_loss = loss_fct(shift_logits.view(shift_logits.size(0), shift_logits.size(1), shift_logits.size(-1))[active_loss], shift_labels[active_loss])
                    samplelen = active_loss.sum(-1)
                    sampleindex = [sum(samplelen[:i]) for i in range(len(samplelen)+1)]
                    exampleloss = [sum(exs_loss[sampleindex[i]:sampleindex[i+1]])/samplelen[i] for i in range(len(samplelen))]
                    exampleloss = [i.item() for i in exampleloss]

                    exids = exi_ids.tolist()
                    exsrcs = source_ids.tolist()
                    extgts = target_ids.tolist()
                    assert len(exids) == len(set(exids))
                for exi, exs, ext, exloss, expred in zip(exids, exsrcs, extgts, exampleloss, predids):
                    if exi in training_loss_log:
                        training_loss_log[exi]['loss'].append(exloss)
                        training_loss_log[exi]['preds'].append([i for i in expred if i != 0])
                    else:
                        training_loss_log[exi] = {}
                        training_loss_log[exi]['loss'] = [exloss]
                        training_loss_log[exi]['preds'] = [i for i in expred if i != 0]
                        training_loss_log[exi]['src'] = [truncate_ids(exs)]
                        training_loss_log[exi]['tgt'] = [i for i in ext if i != 0]
                ################## end of get loss ##################
            ...
            if args.do_eval:
                ...
                if args.do_eval_bleu:
                    ...
                    if args.task!='conala':
                        ...
                        dev_bleu_em = 0.1+cur_epoch
                    # dev_bleu_em = dev_bleu
                    if dev_bleu_em > best_bleu_em:
                        not_bleu_em_inc_cnt = 0
                        log_dist("  [%d] Best bleu+em: %.2f (bleu: %.2f, em: %.2f)"%(cur_epoch, dev_bleu_em, dev_bleu, dev_em), ranks=[0], level=logging.INFO)
                        log_dist("  " + "*" * 20, ranks=[0], level=logging.INFO)
                        best_bleu_em = dev_bleu_em
                        if is_rank_0():
                            f_summary.write("[%d] Best bleu+em changed into %.2f (bleu: %.2f, em: %.2f)\n" % (cur_epoch, best_bleu_em, dev_bleu, dev_em))
        ...
        ################## store loss ##################
        training_loss_log = sorted(training_loss_log.items(), key=lambda x:x[0])
        training_loss_log = dict(training_loss_log)
        with open(os.path.join(args.output_dir, 'training_loss.json'),'w+') as f:
            f.write(json.dumps(training_loss_log))
